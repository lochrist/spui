...
class Todo {
    constructor(title) {
        this.title = valueStream(title);
        this.completed = valueStream(false);
    }
}
class Store {
    constructor() {
        this.editing = valueStream(null);
        this.todoCount = valueStream(0);
        this.remaining = valueStream(0);
        this.filterName = valueStream('all');
        this.todos = new ObservableArray();
        this.todoFilter = new Filter(this.todos, this.isTodoFiltered.bind(this));
    ...
    createView() {
        ...
        return h('section', { id: 'todoapp' }, [
            ...
            h('section', { id: 'main', style: { display: visibleIfAnyTodo } }, [
                this.toggleAllElement = h('input', { id: 'toggle-all', type: 'checkbox', checked: () => this.store.remaining() === 0, onclick: () => this.toggleAll() }),
                h('label', { for: 'toggle-all', onclick: this.toggleAll.bind(this) }, 'Mark all as complete'),
                elementList('ul', { id: 'todo-list' }, this.store.todoFilter.filtered, (listElement, todo) => {
                    ...
                    return h('li', { class: { completed: todo.completed, editing: () => todo === this.store.editing() } }, [
                        ...
                        inputTitleElement = h('input', { class: 'edit', onchange: targetAttr('value', this.store.updateTitle.bind(this.store)) })
                    ]);
                }),
            ]),
